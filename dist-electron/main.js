...
const YubiKeyManager_1 = require("./services/YubiKeyManager");
...
function createWindow() {
    ...
    yubiKeyManager = new YubiKeyManager_1.YubiKeyManager(yubiKeyService);
    premiumService = new PremiumService_1.PremiumService();
...
electron_1.ipcMain.handle('pgp:decrypt-message', async (event, { encryptedMessage, passphrase }) => {
    if (!pgpService) {
        return {
            success: false,
        ...
        try {
            ...
            try {
                ...
                if (pinRetryMatch && pinRetryMatch[1] === '0') {
                    ...
                    if (process.platform === 'darwin') {
                        const tempDir = path.join(electron_1.app.getPath('temp'), 'yubikey-pin-reset');
...
`, { mode: 0o755 });
            ...
            if (errorMsg.includes('Inappropriate ioctl') ||
                ...
                const tempDir = path.join(electron_1.app.getPath('temp'), 'yubikey-interactive');
                if (!fs.existsSync(tempDir)) {
                    fs.mkdirSync(tempDir, { recursive: true });
                }
...
electron_1.ipcMain.handle('yubikey:import-to-gpg', async () => {
    try {
        if (!yubiKeyService) {
            throw new Error('YubiKey service not initialized');
        }
...
electron_1.ipcMain.handle('yubikey:check-public-key', async (_, fingerprint) => {
    try {
        if (!yubiKeyManager) {
...
electron_1.ipcMain.handle('yubikey:import-from-keyserver', async (_, fingerprint) => {
    try {
        if (!yubiKeyManager) {
            throw new Error('YubiKey Manager not initialized');
        }
...
electron_1.ipcMain.handle('yubikey:import-from-file', async () => {
    try {
        if (!yubiKeyManager) {
            throw new Error('YubiKey Manager not initialized');
        }
...
electron_1.ipcMain.handle('yubikey:export-to-file', async (_, fingerprint) => {
    try {
        if (!yubiKeyManager) {
            throw new Error('YubiKey Manager not initialized');
        }
...
electron_1.ipcMain.handle('yubikey:upload-to-keyserver', async (_, fingerprint) => {
    try {
        if (!yubiKeyManager) {
            throw new Error('YubiKey Manager not initialized');
        }
...
electron_1.ipcMain.handle('yubikey:has-public-key-url', async () => {
    try {
        if (!yubiKeyService) {
            throw new Error('YubiKey service not initialized');
        }
...
electron_1.ipcMain.handle('yubikey:get-public-key-url', async () => {
    try {
        if (!yubiKeyService) {
            throw new Error('YubiKey service not initialized');
        }
...
electron_1.ipcMain.handle('yubikey:import-from-card-url', async () => {
    try {
        if (!yubiKeyService) {
            throw new Error('YubiKey service not initialized');
        }
...
electron_1.ipcMain.handle('yubikey:test-functions', async () => {
    try {
        if (!yubiKeyManager) {
            throw new Error('YubiKey Manager not initialized');
        }
        ...
        if ((isPGPMessage || isPGPSignedMessage) && pgpService) {
            ...
            try {
                ...
                if (defaultKey) {
                    ...
                    const publicKeyPath = path.join(electron_1.app.getPath('userData'), 'pgp-keys', `${defaultKey.fingerprint}.public`);
...
electron_1.ipcMain.handle('premium:get-status', async () => {
    try {
        if (!premiumService) {
            return {
                success: false,
...
electron_1.ipcMain.handle('premium:get-bitcoin-address', async (_, { email }) => {
    try {
        if (!premiumService) {
            return {
                success: false,
...
electron_1.ipcMain.handle('premium:check-payment', async (_, { forceCheck = false } = {}) => {
    try {
        if (!premiumService) {
...
electron_1.ipcMain.handle('premium:set-status', (_, { status }) => {
    try {
        if (!premiumService) {
            return {
                success: false,
...
electron_1.ipcMain.handle('premium:get-xpub', () => {
    try {
        if (!premiumService) {
            return {
                success: false,
